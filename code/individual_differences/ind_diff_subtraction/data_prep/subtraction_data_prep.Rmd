---
title: "Subtraction Data Preparation"
...
This script loads the subtraction data for the 2023 quitting project, cleans it and constructs the necessary variables for analysis. 
The dataset gets split into a training and testing dataset (50/50 split, specified in the script "load_subtraction_data_new.R". The desired dataset can be specified in chunk 2 ("Load data"). 
...
# Loading data
```{r Load data}
source("load_subtraction_data.R")
all_logs %<>%
...
user_ids_subtraction <- all_logs[, unique(user_id)]
saveRDS(user_ids_subtraction, file = "~/research-collaboration/quitting_ind_differences/data/subtraction/user_ids_subtraction.Rdata")
```
...
# Here is the problem: there are 223741 cases where one session lasts more than 1 day. 
length(unique(dat1[dat1$time_diff > 1]$user_id))
# This affects 78784 unique users in the training dataset 
...
```{r Session counter}
# long vector of session number pasted with the learning ID, so that each session has a unique value per user. 
...
# Only a few (n = 28) problematic cases 
length(unique(dat1_new[dat1_new$time_diff > 1]$user_id))
# This affects 26 unique users in the training dataset 
...
hist(all_logs[session_count < 100, session_count], 
     ...
     col = adjustcolor("turquoise4", alpha.f = 0.5), 
     border = "turquoise4", 
     xlab = "Session Count",
     density = 30)
```
...
I make a categorical sequential error variable based on error_seq, with 4 levels: 0 = 0; 1 = 1; 2 = 2; 3 = 3; >3 = 4
...
```{r Ability variables}
load("~/research-collaboration/quitting_ind_differences/data_clean/player_ratings_subtraction.Rdata")
...
# all_logs[, non_deliberate := ifelse(quit == 0 & quit_all == 1, 1, 0)] # not sure why this does not work
...
```{r Save}
saveRDS(all_logs, file = "~/research-collaboration/quitting_ind_differences/data/subtraction/subtraction_clean.RData")
```
